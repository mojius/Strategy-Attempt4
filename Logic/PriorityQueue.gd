...
"""
Priority Queue. Min heap priority queue that can take a Vector2i and its
corresponding cost and then always return the Vector2i in it with
the lowest cost value.
Based on: https://en.wikipedia.org/wiki/Binary_heap
Thanks @_bigturtle on Reddit!
"""

var _data: Array = []

func insert(element: Vector2i, cost: float) -> void:
